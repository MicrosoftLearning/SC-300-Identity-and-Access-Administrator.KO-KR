...
1. 다음 설정을 할당합니다.
  - **작업 함수 역할**
  - **역할**: 가상 머신 관리자 로그인
  - **멤버**: 사용자, 그룹, 서비스 주체를 선택합니다.  그런 다음 **+ 구성원 선택**을 사용하여 **사용자2**를 VM의 특정 사용자로 추가합니다.

1. **검토 + 할당**을 선택하여 프로세스를 완료합니다.

#### 작업 3 - Microsoft Entra ID 로그인을 허용하도록 가상 머신 업데이트
...
1. 가상 머신을 설정할 때 생성한 관리자(vmEntraAdmin) 사용자 이름과 비밀번호를 사용합니다.
   - 메시지가 표시되면 예라고 말하여 가상 머신 또는 RDP 세션에 대한 액세스를 허용합니다.

1. 가상 머신이 열리고 모든 소프트웨어가 로드되기를 기다립니다.
...
1. **메모장**을 사용하여 방금 복사한 새 버전의 RDP 파일을 편집합니다. 파일의 아래쪽에 이 두 줄의 텍스트를 추가합니다.
     ```
...
1. 로그인 대화 상자에서 다음 정보를 입력합니다.
   - 사용자 이름 = **AzureAD\User2@ 도메인 이름**
   - 암호 = 랩 공급자가 제공한 암호 입력

   참고: User2는 작업 1에서 관리자로 로그인할 수 있도록 액세스 권한을 부여한 사용자입니다.

1. Windows에서 로그인을 확인하고 일반 데스크톱으로 열립니다.

#### 작업 6 -- Microsoft Entra ID 로그인을 탐색하는 선택적 테스트

1. User2가 관리자 그룹에 추가된 유일한 사용자인지 확인합니다.

1. 시작 단추에서 보조 마우스 클릭을 사용한 다음 팝업 메뉴에서 **컴퓨터 관리**를 선택합니다.

1. **로컬 사용자와 그룹**을 연 다음, **그룹, 관리자**를 탐색합니다.

1. 목록에 **Azure\User2....** 가 표시되어야 합니다.
...
1. 다른 Microsoft Entra ID 회원으로 로그인해 보세요.
