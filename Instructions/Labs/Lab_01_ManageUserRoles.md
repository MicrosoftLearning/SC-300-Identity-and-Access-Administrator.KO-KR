...
3. 왼쪽 탐색 메뉴의 **관리**에서 **사용자**를 선택한 다음, **+ 새 사용자**와 **새 사용자 만들기**를 선택합니다.
...
3.  **Azure Active Directory**에서  **역할 및 관리자**를 선택한 다음, 목록에서 **애플리케이션 관리자** 역할을 선택합니다.
...
2. 이전에 사용하지 않은 경우 Azure AD PowerShell 모듈을 추가하고 가져와야 합니다.  다음 두 명령을 실행하고 확인하라는 메시지가 표시되면 Y 키를 누릅니다.
...
### 연습 5 - Azure Active Directory에서 사용자 제거
