...
3. 왼쪽 탐색 메뉴의 사용자 아래에서 **모든 사용자를** 선택한 **다음+ 새 사용자 및 **새 사용자**** 만들기를 선택합니다.****
...
2. Microsoft Entra 관리 센터를 [https://entra.microsoft.com](https://entra.microsoft.com) Chris Green으로 엽니다.
...
4. 둘러보기 대화 상자**가 **표시되면 [나중에** 가능] **단추를 선택합니다.
...
1. Global 관리istrator 역할로 아직 로그인하지 않은 경우 Microsoft Entra 관리 센터를 열고 로그인합니다.
2. ID로 이동한 다음 사용자 페이지를 선택합니다.
3. 메뉴의 **관리 섹션에서 모든 사용자를** 선택합니다.
...
**참고** - 랩 환경에서 이미 Microsoft Entra ID Premium P2를 활성화한 경우 PIM(Privileged Identity Management)이 사용하도록 설정되고 다음**을 선택하고 **이 사용자에게 영구 역할을 할당해야 합니다.
...
1. 새 InPrivate 브라우저 창을 시작합니다.
2. Microsoft Entra 관리 센터를 [https://entra.microsoftcom](https://entra.microsoft.com) Chris Green으로 엽니다.
...
6. **+ 새 애플리케이션** 선택
7. "**+ 사용자 고유의 **애플리케이션 만들기**가 회색으로 표시되지 않습니다.  갤러리 앱을 **선택하면 만들기** 단추를 사용할 수 있습니다.
...
1. 아직 전역 관리 로그인하지 않은 경우 Microsoft Entra 관리 센터를 시작하고 지금 로그인합니다.
2. 검색 상자에 역할을 입력 **한** 다음 Microsoft Entra ID 역할 및 관리를 시작합니다.
3. 역할 및 관리자 **의**모든 역할 **에서**목록에서 애플리케이션 관리자** 역할을 선택합니다**.
...
1. Microsoft Entra ID 메뉴에서 먼저 ID를 연 **다음 사용자를** 선택한 **다음 모든 사용자를** 선택합니다**.**
...
7. 모든 새 사용자에게 공통 임시 암호를 할당하려면 다음 명령을 실행하고 사용자에게 제공하려는 암호로 바꿉 <Enter a complex Password> 다.  
...
Microsoft Entra ID 페이지를 사용하여 사용자를 추가하고 제거할 수 있습니다.  스크립트를 사용하여 사용자를 만들고 역할을 할당할 수도 있습니다.  스크립트를 사용하여 Chris Green 사용자 계정에 다른 역할을 제공해 보세요. 
...
3. **모든 사용자** 목록을 열고 삭제할 사용자의 검사 상자를 선택합니다. 예를 들어 **Chris Green**을 선택합니다.
...
1. 사용자 페이지의 왼쪽 탐색 영역에서 모든 사용자를** 선택하고 **삭제된 사용자를** 선택합니다**.
...
2. 왼쪽 탐색 창의 ID 아래에서 **사용자를** 선택한 **다음 모든 사용자를** 선택합니다**.**
...
8. 왼쪽 탐색의 **모든 사용자**로 돌아가서 ID** 아래에서 **사용자를 선택합니다 **.**
